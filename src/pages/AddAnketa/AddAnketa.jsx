...
import { countries, statuses } from '../users/User.jsx';
...
const AddAnketa = ({ isEdit = false, userId, initialFormData = initialData }) => {
	const {
		register,
		handleSubmit,
		control,
	...
	const { loading: loadingAddClient, request: addClientRequest } = useRequest('post');
	const { loading: loadingEditClient, request: editClientRequest } = useRequest('put');
	...
	const validate = data => {
		...
		if (!data.mother.name && !data.father.name && !data.contact.name) {
			toast.error('Заполните данные одного из родителей или близкого друга');
			formValid = false;
		}
		return formValid;
	};
	...
	const onSubmit = async data => {
		...
		if (isEdit) {
			await editClientRequest(`/workers/client/${userId}/`, formData);
		} else await addClientRequest('/workers/client_add/', formData);
		reset();
		setTimeout(() => {
			navigate('/');
		}, 3000);
	...
	return (
		<form className='addAnketa' onSubmit={handleSubmit(onSubmit)} encType='multipart/form-data'>
			<div className='addAnketa_top'>
				...
				<div className='addAnketa_top-middle'>
					<input
						...
						onChange={handleChangeOnlyWords}
					/>
			...
			<div className='btns-end'>
				...
				{isEdit ? (
					<Button className='submit-button btn' isLoading={loadingEditClient} type='submit'>
						Сохранить
					</Button>
				) : (
					<Button className='submit-button btn' isLoading={loadingAddClient} type='submit'>
						Сохранить
					</Button>
				)}
			</div>
